...
class Instance:
    ...
    def compute_bounds(self) -> 'None':
        ...
        self.min_path = int(max([compute_path(self.distances[o, i], [o, i], min_select, k) for i in range(self.n)],
                                key=lambda b: b['c'])['c'])
        ...
        self.max_path = int(min([compute_path(self.distances[o, i], [o, i], max_select, k) for i in range(self.n)],
                                key=lambda b: b['c'])['c'])
    ...
    def save_dzn(self, file_path=None):
        distances_list = self.distances
        distances_str_arr = [", ".join([str(int(i)) for i in distances_list[j]]) for j in range(len(distances_list))]
        distances_str = '\n                      | '.join(distances_str_arr)
        instance = f'''m = {self.m};
